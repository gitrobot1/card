...
import { useDouNiuRoomSocket } from '../composables/useDouNiuSocket'
import { showToast } from '../composables/useToast'
import type { DouNiuRoom } from '../types/douniu'

const router = useRouter()
const route = useRoute()

const room = ref<DouNiuRoom | null>(null)
...
const invitePath = computed(() => {
  ...
  return `/games/douniu/online?room=${room.value.id}`
})
...
function goToGame(next: DouNiuRoom) {
...
async function refreshRoom() {
  ...
  try {
    const next = await fetchDouNiuRoom(room.value.id)
    room.value = next
    const me = next.players.find((p) => p.user_id === selfUserId.value)
    selfReady.value = me?.ready ?? false
    goToGame(next)
...
async function enterRoom() {
  ...
  try {
    ...
    room.value = await joinDouNiuRoom(inviteRoomId)
...
async function toggleReady() {
  ...
  try {
    room.value = await readyDouNiuRoom(room.value.id, nextReady)
...
async function handleStart() {
  ...
  loading.value = true
  try {
    room.value = await startDouNiuRoom(room.value.id)
    goToGame(room.value)
  } catch (err) {
...
async function handleLeave() {
  if (!room.value) {
    router.push('/games/douniu')
    return
  }
  loading.value = true
  try {
    await leaveDouNiuRoom(room.value.id)
  } catch {
    // ignore
  } finally {
    loading.value = false
