...
const allReady = computed(() => {
  const players = room.value?.players ?? []
  return playerCount.value >= 2 && players.length > 0 && players.every((p) => p.ready)
})
const canStart = computed(() => isHost.value && allReady.value && room.value?.status === 'waiting')
    ...
    <section class="hero">
      <div class="hero__top">
        <div>
          ...
          <h1>等待房间</h1>
          <p class="hero__desc">2-8 人，全员准备后由房主开始</p>
        </div>
    ...
    <section v-if="room" class="ddz-room__panel">
      ...
      <div class="ddz-room__slots ddz-room__slots--8">
        ...
        >
          ...
          <span v-else-if="player" class="zjh-room__waiting">未准备</span>
        </div>
      </div>
      ...
      <p v-else-if="!allReady" class="ddz-room__hint">当前 {{ playerCount }} 人，等待全员准备</p>
      <p v-else-if="isHost" class="ddz-room__hint">全员已准备，点击开始游戏</p>
      <p v-else class="ddz-room__hint">全员已准备，等待房主开始</p>
