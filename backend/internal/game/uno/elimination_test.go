...
func TestMultiPlayerEmptyHandEliminates(t *testing.T) {
	...
	if len(events) == 0 || events[len(events)-1].Type != EventPlayerOut {
...
func TestVoteEndFinishesWhenAllActiveAgree(t *testing.T) {
	g := readyGame(t, 2)
	g.CanVoteToEnd = true
	g.Players[0].Hand = []Card{{ID: "a", Color: ColorRed, Value: "1", Label: "1"}}
	g.Players[1].Hand = []Card{{ID: "b", Color: ColorRed, Value: "2", Label: "2"}, {ID: "c", Color: ColorRed, Value: "3", Label: "3"}}
	g.Players[2].Hand = []Card{{ID: "d", Color: ColorRed, Value: "4", Label: "4"}, {ID: "e", Color: ColorRed, Value: "5", Label: "5"}, {ID: "f", Color: ColorRed, Value: "6", Label: "6"}}
	g.syncCounts()
	var events []GameEvent
...
func TestLastStandingGetsWorstRank(t *testing.T) {
	...
	g.Players[2].Hand = []Card{{ID: "x", Color: ColorRed, Value: "1", Label: "1"}}
	g.syncCounts()
	var events []GameEvent
